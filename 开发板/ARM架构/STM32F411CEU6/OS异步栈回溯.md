...
> **为什么 FreeRTOS 要改源码？** FreeRTOS V10.3.1 的 TCB 中**没有** `uxSizeOfStack` 字段，CmBacktrace 官方移植要求向 `tasks.c` 追加 `vTaskStackAddr()/vTaskStackSize()/vTaskName()` 三个函数。更优雅的免改源码方案见参考资料（PR #82）。
...
> 免改源码的替代方案：定义 `traceRETURN_xTaskGetCurrentTaskHandle` 宏，在 `xTaskGetCurrentTaskHandle()` 返回时记录当前任务的栈地址/大小（见 PR #82）。
...
A4：CmBacktrace 需要从 TCB 读取"当前任务的栈地址、栈大小、任务名"三个信息。FreeRTOS V10.3.1 的 TCB **没有导出** `uxSizeOfStack` 字段，也没有官方 API 直接取当前任务栈范围，所以要在 `tasks.c` 里补 `vTaskStackAddr()/vTaskStackSize()/vTaskName()`。免改源码的办法是定义 `traceRETURN_xTaskGetCurrentTaskHandle` 宏，在 `xTaskGetCurrentTaskHandle()` 返回时顺带记录。RT-Thread 等 TCB 结构完整暴露栈信息的 RTOS 则不需要改。
...
- [CmBacktrace PR #82](https://github.com/armink/CmBacktrace/pull/82) — 通过 `traceRETURN_xTaskGetCurrentTaskHandle` 免改 FreeRTOS 源码获取任务栈信息
