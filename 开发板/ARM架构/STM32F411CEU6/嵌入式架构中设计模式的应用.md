...
A 1：
1. 不知道
2. 模式是一种可复用的解决办法不是现有的代码，框架是什么意思？
...
A 2：
1. Handle 使用了 callback 回调模式以及观察者订阅模式，在 porting 中使用了 adapter 适配模式，driver 使用了单例模式，port、handle 和 driver 组成工厂模式，而 handle 与 driver 组成桥接模式
2. Handle 解决了同步操作阻塞 cpu 的问题，porting 解决了各层之间函数到注入问题，工厂模式解决了实例管理问题，桥接模式实现了多态
...
A 3：
1. Adapter 是利用数据类型转换来实现的适配，当上层与下层的数据类型不同时需要进行适配；而 bridge 是将一个函数拆分成了抽象和具体两部分，当具体是不确定时，使用 bridge 来实现多态
...
A 4：
1. 在中断或者循环中被反复创建，可能因为重复定义导致内存泄露
2. 其中一个创建后被重新 NULL，并指定地址，原地址就被遗漏了
