...
| 换 RTOS（FreeRTOS → RT-Thread） | 全局替换 RTOS API 调用 | OSAL Impl 目录（1个目录） |
| 换外设芯片（LCD ST77916 → RM69330） | 改 LVGL Port + UI + 状态管理 | Adapter Porting（1个文件） |
| 换业务需求 | 波及底层初始化代码 | APP 层（1个目录） |
