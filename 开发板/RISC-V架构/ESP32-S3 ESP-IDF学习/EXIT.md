### 外部中断简介
外部中断属于硬件中断，由微控制器外部事件触发。微控制器的特定引脚被设计为对特定事件（如按钮按压、传感器信号变化等）作出响应，这些引脚通常称为“外部中断引脚”![[assets/EXIT/file-20260421201554956.png]]
外部中断具备两种触摸类型：
...
void gpio_install_isr_service(esp_intr_alloc_flag_t flags);
	该函数用来注册中断服务
	形参表
		![[assets/EXIT/file-20260421201554960.png]]
esp_err_t gpio_isr_handler_add(gpio_num_t gpio_num,gpio_isr_t isr_handler, void* args);
	该函数设置某个管脚的中断服务函数
	参数表
		![[assets/EXIT/file-20260421201554963.png]]
	定义中断服务的回调函数
...
void gpio_intr_enable(gpio_num_t gpio_num)
	该函数用来配置某个管脚开启外部中断
	参数表和注意事项
		![[assets/EXIT/file-20260421201554965.png]]
